...
const DEFAULT_DDV_RESOURCE_PATH =
...
class MobileWebCapture {
	...
	// DDS (scanner-workflow) view containers MWC owns and lends to the DocumentScanner.
	private ddsViewContainers: HTMLElement[] = [];
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			if (this.scanner) {
				...
				// DDS >= 1.5.0 leaves its scanner view container visible after initialize()
				this.hideDDSViewContainers();
			}
	...
	private setupMWCViews() {
		...
		if (!this.config.container) {
			...
			this.config.container.style.position =
				...
				this.config.container?.style?.position !== "fixed"
					? "relative"
					: this.config.container?.style?.position;
		}
		...
		this.mwcViews = {
			[EnumMWCViews.Library]: {
				config: {
					...
					onViewUploadsHistory: () =>
						this.switchView(EnumMWCViews.History, { caller: EnumMWCViews.Library }),
			...
			[EnumMWCViews.Document]: {
				config: {
					...
					onViewUploadsHistory: () =>
						this.switchView(EnumMWCViews.History, { caller: EnumMWCViews.Document }),
					onClose: () => this.dispose(),
					...
					onTransferPages: (mode, docId, selectedIdx) =>
						this.handleTransferPage(mode, docId, selectedIdx),
					showLibraryView,
				},
			},
			[EnumMWCViews.Page]: {
				config: {
					...
					onViewUploadsHistory: () =>
						this.switchView(EnumMWCViews.History, { caller: EnumMWCViews.Page }),
					onClose: () => this.dispose(),
	...
	private createMWCViewContainers(mainContainer: HTMLElement): Record<string, HTMLElement> {
		...
		const ddsViewContainers = ddsViews.reduce(
			(containers, view) => {
		...
		this.ddsViewContainers = Object.values(ddsViewContainers);

		const mwcViewContainers = mwcViews.reduce(
			(containers, view) => {
				const container = createContainer(view, {
					position: "relative",
					height: "100%",
	 ...
	 */
	private hideDDSViewContainers(): void {
		this.ddsViewContainers.forEach((container) => {
	...
	private switchView(targetView: EnumMWCViews, config?: any) {
		...
		if (targetView === this.currentView) {
			(this.mwcViews[targetView].instance?.setVisible as (a: boolean, b?: any) => void)?.(
				true,
				config,
			);
			return;
