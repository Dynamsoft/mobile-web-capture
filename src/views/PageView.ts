...
export class PageView extends MWCView {
	...
	protected createToolbars(): void {
		...
		const buttons = [
			...
			},
			{
				id: "mwc-page-share",
				icon:
					toolbarButtonsConfig?.share?.icon ||
	...
	private toggleDDVHeaderButtons(enable: boolean): void {
		...
		headerContainer.childNodes.forEach((btn) =>
			(btn as HTMLElement).classList[action]("ddv-disable-button"),
		);
	}

	private disableOtherEditButtons(activeButton: HTMLElement): void {
		this.config.container
			.querySelectorAll(".mwc-view-controls-container")[1]
	...
	private handleAnnotate(): void {
		const annotateButton = this.config.container.querySelector(
			".mwc-annotation-set",
		) as HTMLElement;
