...
export class TransferView extends MWCView {
    ...
    `;
		this.headerTitle = this.MWCViewElements.headerContainer.querySelector(
			".mwc-transfer-view-header-title",
		);
	}
	...
	setVisible(
		visible: boolean,
		config?: { mode: TransferMode; docId: string; selectedIdx: number[] },
	): void {
		...
		if (visible && config) {
			...
			this.toolbarBtn.action.onclick = () =>
				this.handleActionClick(config.docId, config.selectedIdx);

			const docs = DDV.documentManager
				.getAllDocuments()
				.filter((doc) =>
					this.transferMode === TransferMode.Move ? doc.uid !== config.docId : true,
				);
	...
	private updateToolbarStyle(): void {
		const isDisabled =
			this.transferMode === TransferMode.Move
				? this.checkedDocUids.length !== 1
				: this.checkedDocUids.length < 1;
