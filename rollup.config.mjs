...
const external = ["dynamsoft-document-scanner", "dynamsoft-document-viewer"];
	...
	// 6. Type declarations for CommonJS/UMD
	{
		...
		plugins: [
			dts(),
			{
				writeBundle(options, bundle) {
					let txt = fs
						.readFileSync("dist/mwc.d.ts", { encoding: "utf8" })
						.replace(/([{,]) type /g, "$1 ");
		...
		plugins: [
			dts(),
			{
				...
				writeBundle(options, bundle) {
					...
					let txt = fs
						.readFileSync("dist/mwc.bundle.esm.d.ts", { encoding: "utf8" })
						.replace(/([{,]) type /g, "$1 ");
